...
class StanleyNode : public rclcpp::Node {
 public:
  StanleyNode()
    : Node("stanley_node"), angleIMU(0.0), linear_velocity(1.0), currentValuesMotor({4.0, 4.0, 4.0, 4.0}), actual_position({0.0, 0.0, 0.0}), desired_positon({1.0, 1.0, 0.5}) {
    ...
    subscription_actual_angle_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      "/actual_angle", 10, std::bind(&StanleyNode::actual_angle_callback, this, std::placeholders::_1));
    ...
    publisher_delta_angle_ = this->create_publisher<std_msgs::msg::Float64MultiArray>("/delta_angle",
                                                                                      10);
    timer_ = this->create_wall_timer(std::chrono::milliseconds(500), std::bind(&StanleyNode::timer_callback, this));
