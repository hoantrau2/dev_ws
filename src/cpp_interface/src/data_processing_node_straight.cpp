...
class DataProcessingNode : public rclcpp::Node {
 public:
  DataProcessingNode()
    : Node("data_processing_node"), angleIMU(0.0), actual_position({0.0, 0.0}), index(0) {
    initializeArrays();
 ...
 private:
  ...
  void timer_callback() {
    ...
    double arctann = std::atan2(Kp * d_max, Ksoft + 0.4);
    if (arctann > 20 * M_PI / 180)
      arctann = 20 * M_PI / 180;
    else if (arctann < -20 * M_PI / 180)
      arctann = -20 * M_PI / 180;
    if (std::atan2(actual_position[1], actual_position[0]) - theta[index] >= 0)
      message.data[0] = theta[index] - angleIMU - arctann;
    else
      message.data[0] = theta[index] - angleIMU + arctann;

    message.data[1] = d_max;
  ...
  void tf_callback(const tf2_msgs::msg::TFMessage::SharedPtr msg) {
    for (auto transform : msg->transforms) {
      if (transform.child_frame_id == "base_footprint") {
        ...
        message2.data.resize(3); // Set size of data vector to 4
