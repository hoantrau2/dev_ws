...
class MinimalPublisher : public rclcpp::Node 
{
  public:
    MinimalPublisher() 
        : Node("minimal_publisher"), count_(0) 
        {
          publisher_ = this->create_publisher<tutorial_interfaces::msg::MyInterface>("transfer_to_pico_topic", 10); // use create_publisher to create publisher
          timer_ = this->create_wall_timer(500ms, std::bind(&MinimalPublisher::timer_callback, this)); // use create_wall_timer to timer, timer 500ms
                                 // call timer_callback
        }
  ...
  private:
    void timer_callback()
    {
      auto message = tutorial_interfaces::msg::MyInterface();
      message.data = this->count_++; 
      message.first_data = "ros2 hello";

      RCLCPP_INFO(this->get_logger(), "I heard: '%s'",  message.first_data.c_str());
      RCLCPP_INFO(this->get_logger(), "Publishing: '%f'", message.data);
      publisher_->publish(message);
  }
  rclcpp::TimerBase::SharedPtr timer_;
  rclcpp::Publisher<tutorial_interfaces::msg::MyInterface>::SharedPtr publisher_;
  size_t count_;
};
